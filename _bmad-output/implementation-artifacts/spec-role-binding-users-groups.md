...
final_revision: '44b29f70999531dccf245a13037cd31ea7507e0f'
context:
...
- 内置 administrator 角色的"分配用户"按钮禁用（admin 用户不需要角色绑定），可查看但不可修改
...
| 内置 administrator 角色分配用户 | PUT /ovpn/role/{adminRoleId}/users | 返回 400 "内置超管角色不支持分配用户" | - |
...
- [x] `internal/openvpnweb/role.go` -- 新增 `roleAssignUsersHandler(c)` PUT /ovpn/role/:id/users：接收 {userIds:[]}；事务内先把该角色下不在 userIds 中的用户 role_id 设为 NULL，再把 userIds 中的用户 role_id 设为 :id；内置 administrator 角色拒绝（400）；写审计日志 -- 角色用户分配
...
- [x] `internal/openvpnweb/role.go` -- 新增 `roleAssignGroupsHandler(c)` PUT /ovpn/role/:id/groups：接收 {groupIds:[]}；事务内先把该角色下不在 groupIds 中的组 role_id 设为 NULL，再把 groupIds 中的组 role_id 设为 :id；Default 组(ID=1)拒绝修改 role_id（400）；写审计日志 -- 角色用户组分配
...
- [x] `frontend/src/pages/Roles/index.tsx` -- 内置 administrator 角色："分配用户"按钮禁用并 title 提示"内置超管角色不支持分配用户"；"分配用户组"按钮正常可用 -- 内置角色保护
...
- Given 内置 administrator 角色，when 点击"分配用户"，then 按钮禁用，提示"内置超管角色不支持分配用户"
...
- `internal/openvpnweb/role.go` — 新增 role:assign_users / role:assign_groups 权限码；实现 4 个新 handler；roleDeleteHandler 增加用户组检查；roleListHandler 返回 userCount/groupCount；assign handler 添加 administrator 拒绝与禁用角色检查
...
- `frontend/src/pages/Roles/index.tsx` — 列表增加用户数/用户组数列；新增 UserAssignDialog（穿梭框）与 GroupAssignDialog（树形勾选）；administrator 角色按钮禁用；对话框保存时禁止关闭

### Review Findings Breakdown

- **Patches applied**: 7 项（1 高危 + 5 中危 + 1 低危）
- **Items deferred**: 8 项（3 高危 pre-existing + 4 中危 + 1 低危，记录到 deferred-work.md）
- **Items rejected**: 3 项（spec 明确要求返回全量字段、设计决定、误报）

### Follow-up Review Recommendation

true — 本次 review pass 修复了 1 个高危权限提升漏洞（administrator 角色未拒绝分配用户组）和 5 个中危问题，涉及安全、数据完整性、业务逻辑一致性，建议独立 follow-up review 验证修复效果。

### Verification Performed

- `go build ./...` — 编译通过，无错误
- `go vet ./...` — 无 vet 警告
- `cd frontend && npm run build` — 前端构建通过（2921 modules, 1.26s）

### Residual Risks

1. PATCH /ovpn/user 仍可通过 user:update 权限修改 role_id（pre-existing，已记录到 deferred-work）
2. RBAC 缺乏角色层级约束，assign_users 可分配到任意已启用角色（pre-existing 设计局限）
