...
function boundsFor(features: GeoFeature[]): Bounds | null {
  ...
  // Use Web-Mercator units on both axes. The previous implementation measured
  // X in longitude degrees but Y in Mercator radians; wide countries such as
  // Russia therefore had a 360-degree X span versus a ~1-unit Y span and were
  // rendered as a nearly flat horizontal line.
...
function projectionFor(features: GeoFeature[]) {
  ...
  const xSpan = Math.max(0.01, bounds.maxX - bounds.minX);
  ...
  const offsetX = (WIDTH - usedWidth) / 2 - bounds.minX * scale;
  const offsetY = (HEIGHT - usedHeight) / 2 + bounds.maxY * scale;
  return ([longitude, latitude]: Position): Position => [mercatorX(longitude) * scale + offsetX, offsetY - mercatorY(latitude) * scale];
}
...
function administrativeLevelLabel(level: CountryMapLevel) {
  return level === 'adm1' ? '省 / 州 / 大区' : '市 / 县 / 区';
}

function locationHierarchy(country: CountryDrillContext) {
  const target = country.target;
  if (!target) return [country.label, '省 / 州 / 大区', '市 / 县 / 区'];
  const levels = [country.label, target.point.province, target.point.city].filter((item, index, items): item is string => Boolean(item && items.indexOf(item) === index));
  return levels.length > 1 ? levels : [country.label, '省 / 州 / 大区', '市 / 县 / 区'];
}
...
async function fetchHighchartsBoundary(iso3: string, level: CountryMapLevel, parent?: GeoFeature): Promise<GeoFeature[] | undefined> {
  ...
  try {
    ...
    const response = await fetch(`https://code.highcharts.com/mapdata/countries/${countryCode}/${mapKey}.topo.json`);
...
function abortableBoundaryRequest<T>(request: Promise<T>, signal: AbortSignal): Promise<T> {
  if (signal.aborted) return Promise.reject(new DOMException('请求已取消', 'AbortError'));
  return Promise.race([
    request,
    new Promise<T>((_, reject) => signal.addEventListener('abort', () => reject(new DOMException('请求已取消', 'AbortError')), { once: true })),
  ]);
}
...
async function fetchBoundary(iso3: string, level: CountryMapLevel, parent: GeoFeature | undefined, signal: AbortSignal): Promise<GeoFeature[]> {
  ...
  let request = boundaryCache.get(key);
  if (!request) {
    // Do not bind a shared cached download to an individual component's abort
    // signal. React Strict Mode immediately cleans up the first effect; doing
    // so previously aborted and permanently poisoned this cache entry, leaving
    // a blank country grid even though the boundary endpoint was reachable.
    request = (async () => {
      const compactFeatures = await fetchHighchartsBoundary(iso3, level, parent);
      ...
      const response = await fetch(`/ovpn/dashboard/geo-boundary/${encodeURIComponent(iso3)}/${level.toUpperCase()}`);
    ...
    void request.catch(() => {
      // Failed network requests must be retriable rather than caching an error.
      if (boundaryCache.get(key) === request) boundaryCache.delete(key);
    });
  }
  // Each consumer can still stop waiting when it unmounts, while the shared
  // boundary request continues for a subsequent render or another map view.
  return await abortableBoundaryRequest(request, signal);
}
...
export function CountryUsageMap({ country, markers, emptyMessage, onBackToWorld, onMarkerSelect, loadIPDetails }: CountryUsageMapProps) {
  ...
  // Show administrative names immediately after leaving the globe. The labels
  // stay visible even when there is no current traffic data, so the country
  // map is useful as an explorable province/state → city/county hierarchy.
  const regionLabels = useMemo(() => renderedFeatures
    .map((feature, index) => {
      const center = featureCenter(feature);
      if (!center) return undefined;
      const [x, y] = project(center);
      return { key: `${featureName(feature, index)}-${index}`, name: featureName(feature, index), x, y };
    })
    .filter((label): label is { key: string; name: string; x: number; y: number } => Boolean(label))
    // Country ADM1 maps are typically small; retain a cap for fallback data
    // sets so densely subdivided maps remain readable and responsive.
    .slice(0, 120), [project, renderedFeatures]);
  const hierarchy = useMemo(() => locationHierarchy(country), [country]);
  ...
  return <div ref={mapRef} className="country-usage-map relative isolate h-[320px] overflow-hidden rounded-[1.35rem] border" aria-label={`${country.label}行政区使用分布图`}>
    ...
    <svg ref={svgRef} className="relative z-[1] h-full w-full touch-none select-none cursor-grab active:cursor-grabbing" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" tabIndex={0} aria-label={`${country.label}行政区地图，支持下钻、拖动平移和缩放`} onWheel={(event: ReactWheelEvent<SVGSVGElement>) => { event.preventDefault(); zoom(event.deltaY < 0 ? 1.2 : 1 / 1.2, event.clientX, event.clientY); }} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={finishDrag} onPointerCancel={finishDrag}>
      ...
      <g transform={`translate(${viewport.translateX} ${viewport.translateY}) scale(${viewport.scale})`}>
        ...
        <g className="country-map-labels pointer-events-none" aria-hidden="true">
          {regionLabels.map((region) => <text key={region.key} x={region.x} y={region.y} textAnchor="middle" dominantBaseline="central" className="fill-foreground/85 text-[18px] font-semibold" style={{ paintOrder: 'stroke', stroke: 'hsl(var(--background) / 0.92)', strokeWidth: 4, strokeLinejoin: 'round' }}>{region.name}</text>)}
        </g>
    ...
    <div className="china-map-control pointer-events-none absolute left-3 top-3 flex max-w-[calc(100%-11rem)] items-center gap-2 rounded-full border px-2.5 py-1.5 text-[11px] font-medium text-foreground shadow-sm backdrop-blur"><MapPinned className="h-3.5 w-3.5 shrink-0 text-primary" /><span className="truncate">{country.label} · {administrativeLevelLabel(level)}</span></div>
    <div className="china-map-control pointer-events-none absolute left-3 top-[3.35rem] z-10 hidden max-w-[calc(100%-11rem)] items-center gap-1 overflow-hidden rounded-lg border px-2.5 py-1.5 text-[10px] text-muted-foreground shadow-sm backdrop-blur sm:flex" aria-label="当前行政区层级">{hierarchy.map((item, index) => <span key={`${item}-${index}`} className="inline-flex min-w-0 items-center gap-1"><span className="truncate">{item}</span>{index < hierarchy.length - 1 && <span className="shrink-0 text-primary/70">/</span>}</span>)}</div>
