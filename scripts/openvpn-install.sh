...
install_openvpnweb() {
    ...
    ARCHIVE="openvpn-web-linux-${ARCH}.tar.gz"
    DOWNLOAD_DIR="$(mktemp -d)"
    wget "https://github.com/${OPENVPN_WEB_REPO}/releases/latest/download/${ARCHIVE}" -O "$DOWNLOAD_DIR/$ARCHIVE"
    tar -xzf "$DOWNLOAD_DIR/$ARCHIVE" -C "$DOWNLOAD_DIR"
    install -m 0755 "$DOWNLOAD_DIR/openvpn-web" /usr/local/bin/openvpn-web
    rm -rf "$DOWNLOAD_DIR"
}
