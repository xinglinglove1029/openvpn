...
- **交互式地理态势**：可切换“在线客户端来源 / 管理操作来源 / 网站目标服务”，支持全球可旋转地球、中国行政区钻取地图（省级 → 地级市 → 区县）、数量标记与去重公网 IP 明细查看。
...
| **地理态势中心** | 全球地球 / 中国地图、区域数量、来源切换、未知区域提示 | 地球可拖拽旋转缩放；中国地图支持省级 → 地级市 → 区县下钻、拖动平移、缩放与悬停详情 |
...
- **中国聚焦**：展示省级行政区、名称、悬浮信息和区域数量；点击省份下钻到地级市、点击城市下钻到区县，支持面包屑返回、拖动平移和缩放。IP 归属统计会严格遵循数据源精度：当前内置 IP 库精确到市级时，区县仅展示行政边界，不会将市级数据错误归入某个区县。
- **三类来源**：
...
| **地理态势** | 全球交互地球、中国行政区钻取地图（省级 → 地级市 → 区县）、在线/审计/网站目标三类来源、区域数量、去重公网 IP 明细分页 | 大屏地理态势中心 |
...
- 地理态势支持全球地球与中国行政区钻取地图（省级 → 地级市 → 区县），切换在线客户端/管理操作/网站目标三种数据来源
