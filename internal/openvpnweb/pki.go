...
// normalizeClientTopology makes profiles generated by older releases compatible
// with the server's subnet address pool. OpenVPN Connect keeps an explicit
// topology net30 from the client profile and may reject a /24-pool address
// before it can apply the server-side settings.
func normalizeClientTopology(content string) string {
	...
	for _, line := range lines {
		...
		if trim != "" && !strings.HasPrefix(trim, "#") && !strings.HasPrefix(trim, ";") && len(fields) > 0 {
			if fields[0] == "topology" {
				if !topologyWritten {
					result = append(result, "topology subnet")
					topologyWritten = true
				}
				continue
			}
			if fields[0] == "dev" && len(fields) > 1 && fields[1] == "tun" {
				devTunIndex = len(result)
			}
		}
...
`, protoLine, serverAddr, port, serverName, authUserPass, mfaLine, ipv6Lines, explicitExitNotify,
	...
	return os.WriteFile(filepath.Join(clientsDir, name+".ovpn"), []byte(normalizeClientTopology(ovpnContent)), 0644)
}
