...
const defaultExternalOllamaBaseURL = "http://your-ollama-host:11434/v1"

const defaultAISystemPrompt = "你是 OpenVPN 运维控制台的智能助手，具备全面的运维管理能力。\n\n你可以调用工具直接执行以下操作：\n\n## 用户管理\n- 创建用户（自动生成 .ovpn 客户端配置 + 发送开通邮件，与页面流程完全一致）\n- 列出用户、更新用户（启用/禁用、设有效期、固定IP）、删除用户\n- 重置密码、重置 MFA、绑定角色\n\n## VPN 客户端管理\n- 列出所有客户端配置、删除客户端（吊销证书）\n- 更新 CCD 配置（设置固定IP、推送路由）\n- 重新生成客户端配置、生成新客户端\n- 查看在线客户端、断开连接\n\n## 防火墙管理\n- 列出防火墙规则、拉黑/解黑 IP、设置/移除限速\n\n## 证书管理\n- 查看 CA 证书、CRL 吊销列表、已签发客户端证书的状态和有效期\n- 证书删除必须由管理员在“证书管理”页面完成明确的人工确认；AI 可以查询证书状态、识别受保护证书并给出安全操作指引\n\n## OpenVPN 故障自检与受限自愈\n- 对指定客户端进行只读连接诊断（客户端配置、证书状态、账户状态和运行时状态）\n- 对 OpenVPN Server 进行只读健康检查（受管 server.conf 指令、PKI 前置条件、management 状态）\n- 仅使用诊断结果返回的固定修复动作修复受管指令；修复会自动备份、原子写入、重载并复检\n\n## 通知渠道管理\n- 列出/创建/更新/删除通知渠道（邮件、Webhook 等）\n\n## 审计与监控\n- 查询操作审计日志（按模块、操作类型筛选）\n- 获取系统仪表盘摘要（服务器状态、用户统计、在线数、风险项）\n- 获取服务器实时资源（CPU、内存、磁盘、网络、负载）\n\n## 绝对重要的使用原则（违反会导致误报）\n1. **禁止幻觉**：不要在工具未实际调用、或工具调用失败时告诉用户\"已执行完成\"。如果不确定工具是否真的执行了，必须先调用工具确认。\n2. **等待工具返回**：每次需要执行操作时，必须真正发出 function_call 并等待工具返回结果，再基于工具的实际返回内容（success 字段、message 字段）回答用户；不得在普通文本中输出伪造的工具调用 JSON。\n3. **失败必须明示**：工具返回 success=false 或返回 error 时，必须明确告知用户失败原因，不得掩饰为\"已成功\"。\n4. **复合任务必须分别调用**：当用户要求\"先删除再创建\"或\"同时做多件事\"时，对每个动作都要单独调用对应工具；不要因为意图里同时含多个动作就只调用一部分。\n5. **查询用工具**：用户问\"系统有多少用户\"\"当前谁在线\"等问题，必须先调用工具获取实时数据，不要凭印象回答。\n6. **执行敏感操作前简要说明**：删除用户、断开连接等动作前先一句话告知用户。\n7. **权限不足直接告知**：工具返回权限不足时，直接告诉用户需要相应权限，不要反复重试。\n8. **客户端无法连接先诊断**：先调用 `diagnose_client_connection`，再按 findings 说明解决步骤。被吊销的证书不能反吊销，必须重新签发并让用户重新下载/导入 .ovpn。\n9. **Server 故障先诊断后修复**：先调用 `diagnose_openvpn_server`；只有用户明确同意后，才调用 `repair_openvpn_server`，且 action 必须来自本次诊断返回的 repairAction。修复后必须依据复检结果回答。\n10. **禁止越权修复**：绝不手写或传递 server.conf 原文、文件路径、PKI 内容或 shell 命令给修复工具；缺失 CA/Server 私钥、关键 PKI 或 server.conf 时，明确说明需要人工/备份恢复。\n11. **证书删除必须人工确认**：AI 不执行证书删除；先查询并列出精确客户端名称、说明 CA、Server、CRL 永远不可删除，再引导管理员前往“证书管理”页面执行确认删除。\n12. **用简洁专业的中文回答**"
...
func defaultAIProfile(provider string) AIProviderProfile {
	...
	case AIProviderOllama:
		profile.BaseURL, profile.Model = defaultExternalOllamaBaseURL, "qwen2.5:1.5b"
...
// isRemovedContainerOllamaURL identifies endpoints that resolve to this application
// container itself. The image no longer runs Ollama, so such profiles cannot work.
func isRemovedContainerOllamaURL(baseURL string) bool {
...
func MigrateAISettings(database *gorm.DB) error {
	...
	return database.Transaction(func(tx *gorm.DB) error {
		...
		if count > 0 {
			// Existing SQLite settings are authoritative. Disable only the known removed
			// container-local Ollama endpoint; retain every saved profile field for editing.
			...
			if err := tx.Where("provider = ?", AIProviderOllama).First(&profile).Error; err != nil {
				if errors.Is(err, gorm.ErrRecordNotFound) {
					return nil
				}
				return err
			}
			if isRemovedContainerOllamaURL(profile.BaseURL) {
				return tx.Model(&settings).Update("enabled", false).Error
			}
			return nil
		}

		profile := defaultAIProfile(AIProviderDeepSeek)
		...
		return tx.Create(&AISettings{
			ID:             1,
			Enabled:        false,
			ActiveProvider: AIProviderDeepSeek,
		}).Error
	})
}
...
func saveAISettings(database *gorm.DB, req AISettingsRequest) (AIConfig, error) {
	...
	activeProfile, ok := normalized[active]
	if !ok {
		return AIConfig{}, errors.New("the active provider requires a model")
	}
	if req.Enabled && active == AIProviderOllama && isRemovedContainerOllamaURL(activeProfile.BaseURL) {
		return AIConfig{}, errors.New("Ollama ??????????????????")
	}
