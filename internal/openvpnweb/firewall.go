	...
	case http.MethodPost:
		...
		case "add_ovips":
			...
			for _, f := range fs {
				...
				if err != nil {
					logger.Error(context.Background(), err.Error())
					c.JSON(http.StatusInternalServerError, gin.H{"message": "添加防火墙地址失败"})
					return
				}
			}
			...
			c.JSON(http.StatusOK, gin.H{"message": "防火墙地址已添加"})
		case "delete_ovips":
			username := c.PostForm("username")
			vip := c.PostForm("vip")
			vip6 := c.PostForm("vip6")
			...
			for _, f := range fs {
				...
				if err != nil {
					logger.Error(context.Background(), err.Error())
					c.JSON(http.StatusInternalServerError, gin.H{"message": "删除防火墙地址失败"})
					return
				}
			}
			...
			c.JSON(http.StatusOK, gin.H{"message": "防火墙地址已删除"})
		default:
			var f Firewall
			c.ShouldBind(&f)
